...
import { getTeamData, findTeamIdBySlug } from "@lib/team/nonOrgTeamHelpers";
import {
  getEventTypeHosts,
  getProfileFromEvent,
  getUsersFromEvent,
...
export async function getCRMData(
  ...
  // CRM contact-owner lookup is an enterprise feature and is unavailable in
  // this deployment; any values supplied on the URL are used as-is.
  void crmLookupDone;
...
export async function getTeamId(teamSlug: string, orgSlug: string | null): Promise<number | null> {
  return findTeamIdBySlug(teamSlug, orgSlug);
}
